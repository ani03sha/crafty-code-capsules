    ...
    return (
        <button
            ...
            className="group flex items-center justify-center gap-2 h-[3rem] w-[8rem] bg-gray-900 text-white 
                    ...
                    disabled:scale-100 disabled:opacity-65 dark:bg-white dark:bg-opacity-10"
            disabled={pending}>
            {
                pending ? <div className="h-5 w-5 animate-spin rounded-full border-b-2 border-white"></div> : (
                    <>
                        Submit <FaPaperPlane
                            className="text-xa opacity-70 transition-all group-hover:translate-x-1 group-hover:-translate-y-1" />
                    </>
                )
            }
        </button>
